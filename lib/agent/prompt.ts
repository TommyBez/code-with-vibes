 ...
 * the sandbox firewall rewrites the `Authorization` header on all GitHub egress,
 * so the agent runs plain `git`/`curl` with NO credentials in its commands and
 * the PAT never lives inside the VM.
 */
...
GitHub authentication is handled transparently by the sandbox firewall. Run plain git and curl with NO tokens, credentials, or Authorization headers anywhere — the firewall injects them on the way out. The \`origin\` remote is already set to the plain HTTPS URL, so just:
\`\`\`bash
...
\`\`\`
Then open the PR via the GitHub REST API — again with NO Authorization header:
\`\`\`bash
